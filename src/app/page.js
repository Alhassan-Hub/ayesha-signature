...
export default function Home() {
  ...
  // State to trigger the About Modal from the Home page
  ...
  return (
    <main className="relative w-full overflow-hidden font-sans bg-[#FAF9F6] text-[#2C2424]">
      {/* Pass the state to Navbar so they stay synced */}
      <Navbar externalOpenState={isAboutOpen} setExternalOpenState={setIsAboutOpen} />

      <div className="relative z-10 pt-32">
        
        {/* HERO SECTION */}
        <section className="relative h-[75vh] flex flex-col items-center justify-center text-center px-4">
          <div className="reveal-text z-10"> 
            <h1 className="text-6xl md:text-9xl font-light tracking-tighter mb-4">Ayesha's<span className="block font-serif font-bold italic lowercase text-6xl md:text-9xl mt-2 text-[#DDA7A5] drop-shadow-sm">signature</span></h1>
            <p className="text-[10px] md:text-xs tracking-[0.5em] uppercase opacity-60 mt-8 font-bold text-[#2C2424]">Elegance in every thread</p>
          </div>
        </section>
        ...
        <section className="min-h-[90vh] flex flex-col items-center justify-center px-6 md:px-16 lg:px-24 mb-24">
          <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-start gap-12 md:gap-24">
            ...
            <div className="w-full md:w-3/5 reveal-text order-2 md:order-1" ref={storyLeftRef}>
              <p className="text-[#DDA7A5] text-[10px] font-bold uppercase tracking-[0.4em] mb-4">Behind the Brand</p>
              ...
              <div className="space-y-6 text-sm md:text-base opacity-90 leading-relaxed font-light text-gray-700">
                <p>
                  Isata Barrie is a passionate entrepreneur and final-year student at <strong>IPAM</strong>, specializing in Procurement and Supply Chain Management. Her academic foundation, paired with her work at <strong>Watu Simu</strong>, drives the excellence behind Ayesha's Signature.
                </p>
                <p>
                  As the founder, Isata has built a brand rooted in creativity and authenticity. Proudly <strong>Fullah</strong> by tribe and <strong>Muslim</strong> by faith, she is guided by values of integrity and service, aiming to inspire the next generation of creators.
                </p>
                <p className="italic border-l-4 border-[#DDA7A5] pl-6 py-2 text-[#2C2424] font-serif text-lg">
                  "Success is measured by the positive impact one makes in the lives of others."
                </p>
              </div>

              <div className="mt-12 flex flex-wrap gap-8 items-center">
                <Link href="/shop" className="inline-block px-12 py-5 bg-[#2C2424] text-white rounded-full uppercase tracking-widest text-[10px] font-bold transition-all hover:bg-[#DDA7A5] hover:scale-105 shadow-xl active:scale-95">
                  Explore The Collection
                </Link>
                <button 
                  onClick={() => setIsAboutOpen(true)} 
                  className="text-[10px] uppercase tracking-widest font-bold border-b-2 border-[#DDA7A5] pb-1 hover:text-[#DDA7A5] transition-all"
                >
                  Read Full Story
                </button>
              </div>
        ...
        <section className="py-24 px-6 md:px-16 lg:px-24 bg-white">
          <div className="max-w-7xl mx-auto">
            <div className="mb-16 reveal-text text-center">
                <h2 className="text-4xl md:text-5xl font-serif font-bold text-[#2C2424]">The Featured Edit</h2>
                <div className="h-1 w-20 bg-[#DDA7A5] mx-auto mt-4"></div>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20"><p className="opacity-50 uppercase tracking-widest text-[10px]">Inventory arriving soon.</p></div>
            ) : (
                ...
                {products.map((product) => (
                  <div key={product.id} className="group flex flex-col bg-[#FAF9F6] p-4 rounded-3xl border border-transparent hover:border-gray-100 hover:shadow-2xl transition-all duration-500">
                    <div className="relative w-full aspect-[4/5] overflow-hidden rounded-2xl bg-gray-50 mb-6 shadow-sm">
                        ...
                        <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-[8px] font-bold uppercase tracking-widest text-[#2C2424]">New</div>
                    </div>
                    ...
                    {/* UPDATED "ORDER NOW" BUTTON (POPS MORE) */}
                    <a 
                      href={`https://wa.me/23272273689?text=${encodeURIComponent(`Hello Ayesha! I'm interested in: ${product.name}`)}`} 
                      target="_blank" 
                      className="mt-auto w-full py-4 text-center bg-[#DDA7A5] text-white text-[9px] font-bold uppercase tracking-[0.2em] hover:bg-[#2C2424] transition-all duration-300 rounded-xl shadow-lg hover:shadow-rose-200"
                    >
                      Order via WhatsApp
                    </a>
                  </div>
        ...
        {/* ... (REVIEWS & FOOTER remain the same, just ensured colors match) ... */}
        
        {/* REVIEWS SECTION */}
        <section className="py-24 px-6 md:px-16 lg:px-24 bg-[#FDF8F5]">
          <div className="max-w-3xl mx-auto text-center reveal-text">
            ...
            <h2 className="text-3xl md:text-4xl font-serif font-bold text-[#2C2424] mb-12">Client Love</h2>
            <div className="relative h-[250px] flex items-center justify-center">
              {liveReviews.map((review, idx) => (
                <div key={idx} className={`absolute transition-all duration-1000 ${activeReview === idx ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
                  <p className="text-lg md:text-xl text-gray-600 italic font-light leading-relaxed mb-8">"{review.text}"</p>
                  <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#2C2424]">{review.name}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* FOOTER */}
        <footer className="bg-[#2C2424] text-white py-20 px-6">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-16">
            <div className="space-y-6">
               <h2 className="font-serif font-bold text-2xl tracking-widest italic text-[#DDA7A5]">signature</h2>
               <p className="text-[10px] opacity-60 uppercase tracking-widest leading-loose">Premium modest fashion and luxury Islamic gifting crafted with intention in Sierra Leone.</p>
            </div>
            <div>
               <h3 className="text-[10px] font-bold uppercase tracking-widest text-[#DDA7A5] mb-8">Navigation</h3>
               <ul className="text-[10px] uppercase tracking-widest space-y-4 opacity-80">
            ...
            <div>
               <h3 className="text-[10px] font-bold uppercase tracking-widest text-[#DDA7A5] mb-8">Contact</h3>
               <p className="text-[10px] uppercase tracking-widest opacity-80 mb-2">WhatsApp: +232 72 273689</p>
               <p className="text-[10px] uppercase tracking-widest opacity-80">Freetown, Sierra Leone</p>
            </div>
          </div>
        </footer>
