...
export default function ShopPage() {
  ...
  // SAFE CART LOADER (Wipes corrupted data instantly to prevent white screens)
  useEffect(() => {
    try {
      ...
      if (savedCart) {
        ...
        if (Array.isArray(parsed)) {
          ...
          localStorage.removeItem('ayeshas_cart'); // Wipe bad data
          setCart([]);
        }
      }
    } catch (e) {
  ...
  // Lock scrolling when modals open
  useEffect(() => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      if (!quickViewProduct && !isCartOpen) {
        setCurrentIndex((prev) => (prev + 1) % customItems.length);
      }
    }, 8000); 
    return () => clearInterval(timer);
  }, [customItems.length, quickViewProduct, isCartOpen]);
  ...
  const checkoutViaWhatsApp = () => {
    ...
    const itemList = cart.map(item => `▪ ${item.qty}x *${item.name || 'Item'}*`).join('\n\n');
  ...
  const getProductDescription = (prod) => {
    ...
    if (name.includes('box') || name.includes('bouquet')) return "A thoughtfully curated gift box featuring a selection of our finest essentials. The perfect intentional gift.";
    if (name.includes('hijab') || name.includes('scarf')) return "A beautiful, high-quality hijab scarf available in a variety of stunning colors. It drapes perfectly, offering effortless everyday elegance.";
    return "A beautiful signature piece from our collection, crafted with care and perfect for your modest lifestyle.";
  };

  return (
    <main className="min-h-screen bg-[#FDF8F5] text-[#2C2424] font-sans relative">
      
      {/* LOCKED NAVBAR */}
      <nav className="fixed top-0 w-full z-[100] bg-white/90 backdrop-blur-md border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="text-[#DDA7A5] text-[10px] font-bold uppercase tracking-[0.2em] hover:text-[#D4AF37] transition-colors">
            <span>←</span> Back
          </Link>
      ...
      <button onClick={() => setIsCartOpen(true)} className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[100] w-14 h-14 bg-[#2C2424] text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-105 transition-transform">
      ...
      {/* CART DRAWER */}
      <div className={`fixed inset-y-0 right-0 w-full md:w-[400px] bg-white z-[200] transform transition-transform duration-500 flex flex-col shadow-[-20px_0_50px_rgba(0,0,0,0.1)] ${isCartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item, idx) => (
              <div key={item.id || idx} className="flex items-center gap-4 bg-[#FDF8F5] p-3 rounded-xl border border-gray-100">
                <img src={item.imageUrl || ''} alt={item.name || 'Item'} className="w-16 h-16 object-cover rounded shadow-sm bg-gray-100" />
                <div className="flex-1">
                  <h3 className="text-sm font-serif font-bold text-[#2C2424] line-clamp-1">{item.name || 'Item'}</h3>
      ...
      {quickViewProduct && (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full sm:max-w-3xl max-h-[85vh] bg-white rounded-t-[2rem] sm:rounded-[2rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-8">
            <button onClick={() => setQuickViewProduct(null)} className="absolute top-4 right-4 z-10 w-10 h-10 bg-white/90 backdrop-blur rounded-full flex items-center justify-center text-gray-800 hover:text-black shadow-md border border-gray-200">✕</button>
            ...
            <div className="flex-1 overflow-y-auto flex flex-col sm:flex-row w-full">
              ...
              <div className="w-full sm:w-1/2 p-6 md:p-8 flex flex-col justify-start">
                ...
                <h2 className="text-2xl font-serif font-bold text-[#2C2424] mb-2 leading-tight">{quickViewProduct?.name || 'Item'}</h2>
      ...
      {/* 3D SHOWROOM */}
      <div className="relative w-full h-[80vh] flex flex-col items-center justify-center z-10 bg-[#FDF8F5] pt-16">
        <div className="absolute top-28 text-center z-20 pointer-events-none px-4">
